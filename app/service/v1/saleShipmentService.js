...
class SaleShipmentService extends Service {
  ...
  async create(params) {
    const { ctx } = this;
    const { Goods, SaleShipment } = ctx.model;
    const {
      goodsCategories,
      goodsName,
      specifications,
    ...
    try {
      ...
      const _goodsId =
        (goodsfind && goodsfind._id) || (goodsSave && goodsSave._id);
  ...
  async update(params) {
    ...
    try {
      ...
      const result = await SaleShipment.update({ _id }, res);
  ...
  async delete(params) {
    ...
    const { SaleShipment } = ctx.model;
    try {
      const res = SaleShipment.remove({ _id: { $in: [ ...params ] } });
